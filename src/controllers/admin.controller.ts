...
export const editarEstadistica = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const config = await db.select().from(configPuntuacion).where(
      and(eq(configPuntuacion.activo, true), lte(configPuntuacion.desde, antes.jornada.fechaInicioJornada ?? new Date()),
          or(isNull(configPuntuacion.hasta), gte(configPuntuacion.hasta, antes.jornada.fechaInicioJornada ?? new Date())))
    )
